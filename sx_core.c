...
char *cmdline;
{
	...
	{
		sv_run_exit = Error("Bad quoting or too much arguments");
	...
	while(argc)
	{
		...
		for(i = 0; i < argc; ++i)
		{
			...
			if(i > 0 && i < argc - 1)
			{
				...
				code = sv_run_exit = Error("Bad multi-command line");
		...
		/* Execute single command */
		if(!skip)
			sv_run_exit = ExeCommand(argc, argv, pipe_in, pipe_out);
...
#ifdef SX_DEBUG
	fprintf(stderr, "[Execute CODE = %d]\n", sv_run_exit);
	fprintf(stderr, "[Execute END]\n");
#endif

	/* Return success or error */
	return (sv_run_exit ? -1 : 0);
}

/* PUSH COMMAND NAME TO RUN
   ------------------------
   Return 0 on success, else -1.
*/
RunPush(name)
char *name;
{
	...
	if((k = strlen(name)) < FILENAME_MAX)
	{
		if((i = KeyCount1st(sv_run_name, SX_MAX_RUN)) < SX_MAX_RUN)
		{
			if((sv_run_name[i] = malloc(++k)))
			{
				strcpy(sv_run_name[i], name);
#ifdef SX_DEBUG
	fprintf(stderr, "[RunPush %d = %s]\n", i, name);
#endif
				/* --> Success */
			}
			else
		...
		{
			return Error("Too many running commands");
		}
	}
	else
	{
...
RunPop()
{
	...
	if((i = KeyCount1st(sv_run_name, SX_MAX_RUN)))
	{
#ifdef SX_DEBUG
	fprintf(stderr, "[RunPop %d = %s]\n", i - 1, sv_run_name[i - 1]);
#endif
		free(sv_run_name[i - 1]);

		sv_run_name[i - 1] = NULL;
	}
}
